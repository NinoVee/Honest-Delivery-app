...
export default async function handler(req, res) {
  if (req.method === 'GET') {
    try {
      ...
      console.error(e);
      return res.status(500).json({ error: 'Failed to list orders. Check your Upstash Redis env vars.' });
    }
  }
  ...
  if (req.method === 'POST') {
    try {
      ...
      console.error(e);
      return res.status(500).json({ error: 'Failed to create order. Check your Upstash Redis env vars.' });
    }
  }
